# download_abide_preprocessed_updated.py
#
# Author: Daniel Clark, 2015
# updated: Neurohackweek 2017, to add group as a specifier
'''
...
def collect_and_download(derivative, pipeline, strategy, out_dir,
    ...
    group : float 
        1 or 2 to get only controls/patients
    Returns
    -------
    None
    ...
    try:
        ...
        group_idx = header.index('DX_GROUP')
    ...
    for pheno_row in pheno_list[1:]:
        ...
        try:
            ...
            row_group = int(cs_row[group_idx])
        except Exception as exc:
        ...
        print 'group: %d' % group
        print 'row_group: %d' % row_group

	# If the group isn't right, skip
        if (group == 1) and (row_group==2):
            continue
       
        if (group == 2) and (row_group==1):
            continue
...
if __name__ == '__main__':
    ...
    parser.add_argument('-x', '--sex', nargs=1, required=False, type=str,
                        help='Participant sex of interest to download only '\
                             '(e.g. \'M\' or \'F\')')
    parser.add_argument('-z', '--group', nargs=1, required=False,
                        type=int, help='diagnosis group '\
                                         '1 for controls'\
                                         '2 for patients')
    ...
    try:
        ...
        group = 3 
        print 'No group specified, getting both groups...'
